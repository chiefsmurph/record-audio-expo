...
class PrivateScreen extends React.Component {
  ...
  render() {
    ...
    let { feed: { public: publicFeed } } = this.props.ApplicationState;
    // console.log('render library', feed)
    publicFeed = publicFeed.map(upload => ({
      ...upload,
      displayText: upload.user.username + ' - ' + upload.name
    }));
    return (
      <>
        <Text style={{ fontSize: 28, marginHorizontal: 20, marginTop: 20, marginBottom: 10 }}>Public Messages {showingProfile}</Text>
        {/* <View
          style={{
            marginVertical: 4,
